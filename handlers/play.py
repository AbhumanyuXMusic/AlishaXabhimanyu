...
def cb_admin_check(func: Callable) -> Callable:
    async def decorator(client, cb):
        ...
        if cb.from_user.id in admemes:
            ...
            await cb.answer("Izin dulu tolol!", show_alert=True)
            return
...
async def ee(client, message):
    ...
    if stats:
        ...
        await message.reply("**Hidupin VCGnya COEG!**")
...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                if message.chat.title.startswith("Channel Music: "):
                    await lel.edit(
                        f"<b>Ingatlah untuk menambahkan @{ASSISTANT_NAME} ke Channel Anda</b>",
                    )
                    pass
                try:
    ...
    try:
        ...
        await lel.edit(
            f"<i>{user.first_name} Asisten gue kok lu banned, Minta admin grup buat masukkan lagi asisten @{ASSISTANT_NAME} ke dalam grup:).</i>"
        )
        return
    text_links=None
    ...
    if audio:
        ...
        try:
            ...
            while j < 5:
                ...
                toxxt += f" └ ⚡ __Powered by 🎧𝕍𝕚𝕣𝕥𝕦𝕒𝕝 𝕄𝕦𝕤𝕚𝕔🎧 A.I__\n\n"
...
async def deezer(client: Client, message_: Message):
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"🎼 **Lagu yang Loe minta Sedang Antri di posisi** `{position}`")
    else:
